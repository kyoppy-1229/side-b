...
　（体験版では順番の縛りなし。導入中に自力で辿り着けばその時点で読めます）
...
- **保存ログの開通条件はモードで違います**（`store/virtualBrowser.js` の
  `canOpenPrivateStoryArchive`）。**体験版は進行に関係なく、住所を組み立てられた時点で読めます**
  （水野の話は手がかりで、鍵ではありません。導入中に自力で辿り着いた場合もそのまま読めます）。
  **本編は従来どおり**、水野が共有したリンクを開いて `bbs_opened` に到達するまで開きません。
  掲示板そのもののデータ・検索仕様（`noindex`／検索に出ない）は両モードとも無変更です。
- 終了画面の「最初から」は、体験版の名前空間を消してリロードします（`trial/flow.js` の `restartTrial`）。
  通常版のセーブには触れません。
- **旧ビルドのセーブは開き直します。** 以前は水野の最後の一行で体験版が終了扱いになっていたため、
  そのセーブのままでは終了画面がブラウザを覆い続けて保存ログに辿りつけません。起動時に
  `reopenUnfinishedTrial()`（`trial/flow.js`、呼び出しは `App.vue`）が「保存ログ未読の終了マーク」を
  外し、水野が書き終えた地点から続きを遊べるようにします。最初からやり直す必要はありません。
