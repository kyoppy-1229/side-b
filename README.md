...
- 「一般Web」の3セクションから、34サイト・962URLを直接開けます。サイト／ページの絞り込み、
...
- **体験版が渡さないもの**は `src/trial/restrictions.js` にまとめてあります。学校アーカイブ／
  卒業記録／掲示板の保存ログ／初期版は、検索にも新規タブの候補にも出ず、アドレスバーからも開けません
  （`store/virtualBrowser.js` の `guardTrialEdition`）。掲示板そのもののデータ・検索仕様は無変更です。
...
閲覧できます。34サイト・約960URLがあり、内容は本編とは無関係な普通のWeb情報です。
